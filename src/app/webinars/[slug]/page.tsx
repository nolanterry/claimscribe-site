...
export default function WebinarDetailPage() {
  ...
  const structuredData = [
    ...
    { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: webinar.title, item: `https://claimscribe.ai/webinars/${webinar.slug}` },
    ] },
  ];
    ...
    <>
      ...
      <main id="main-content">
        ...
        <div className="max-w-5xl mx-auto px-4 py-12">
          <Link href="/webinars" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-emerald-600 mb-8 transition-colors"><ArrowLeft size={14} /> Back to Webinars</Link>

          <div className="grid lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2">
              <div className="flex items-center gap-2 mb-4">
                <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 text-xs font-medium rounded">{webinar.category}</span>
              ...
              <div className="mb-10">
                ...
                <div className="space-y-3">
                  {webinar.topics.map((topic, i) => (
                    <div key={i} className="flex items-start gap-3"><CheckCircle size={18} className="text-emerald-600 mt-0.5 shrink-0" /><span className="text-gray-700">{topic}</span></div>
                  ))}
                </div>
              </div>
              <div className="bg-gray-50 rounded-xl p-6 mb-10">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 bg-emerald-50 rounded-full flex items-center justify-center"><Users size={24} className="text-emerald-600" /></div>
                ...
                <div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {relatedWebinars.map((w) => (
                      <Link key={w.slug} href={`/webinars/${w.slug}`} className="block border border-gray-200 rounded-lg p-5 hover:border-emerald-500 hover:shadow-md transition-all group">
                        ...
                        <h3 className="font-medium text-gray-900 group-hover:text-emerald-600 transition-colors line-clamp-2 text-sm">{w.title}</h3>
              ...
              <div className="sticky top-24 bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                ...
                {registered ? (
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); if (email) { setRegistered(true); const existing = JSON.parse(localStorage.getItem("webinar-registrations") || "[]"); existing.push({ email, name, company, webinar: webinar.slug, date: new Date().toISOString() }); localStorage.setItem("webinar-registrations", JSON.stringify(existing)); } }} className="space-y-3">
                    <div><label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">Full Name</label><input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Jane Smith" className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" /></div>
                    <div><label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-1">Work Email *</label><input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="jane@company.com" className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" /></div>
                    <div><label htmlFor="company" className="block text-sm font-medium text-gray-700 mb-1">Company</label><input id="company" type="text" value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Rapid Response Restoration" className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" /></div>
                    <button type="submit" className="w-full bg-emerald-600 text-white py-3 rounded-lg font-semibold hover:bg-emerald-700 transition-colors flex items-center justify-center gap-2">{isUpcoming ? <>Register Free <ArrowRight size={16} /></> : <>Watch Now <Play size={16} /></>}</button>
