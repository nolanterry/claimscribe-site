...
export const metadata: Metadata = {
  title: "Restoration & Insurance Glossary — Key Terms & Definitions",
  description:
    "Comprehensive glossary of restoration and insurance claims terminology. Xactimate, scope of loss, water damage categories, fire scoping, and mold remediation definitions.",
  openGraph: {
    title: "Restoration & Insurance Glossary — ClaimScribe",
    description: "Essential restoration and insurance terms every contractor and estimator needs to know.",
  },
...
export default function GlossaryIndexPage() {
  ...
  const structuredData = {
    ...
    "@type": "DefinedTermSet",
    name: "Restoration & Insurance Claims Glossary",
    description: "Comprehensive glossary of restoration, insurance claims, and damage scoping terminology.",
    url: "https://claimscribe.ai/glossary",
    hasDefinedTerm: sortedTerms.map((t) => ({
      "@type": "DefinedTerm",
      name: t.term,
      description: t.definition,
      url: `https://claimscribe.ai/glossary/${t.slug}`,
    })),
  };
  ...
  return (
    <>
      ...
      <main id="main-content">
        ...
        <section className="bg-[#080810] py-20 text-center px-4">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-white">Restoration & Insurance Glossary</h1>
          <p className="text-lg text-gray-400 max-w-2xl mx-auto">
            {GLOSSARY_TERMS.length} essential terms every restoration contractor and estimator should know.
          </p>
        </section>
        ...
        <section className="border-b border-gray-200 sticky top-16 bg-white/95 backdrop-blur z-40">
          <div className="max-w-5xl mx-auto px-4 py-3 flex flex-wrap gap-1 justify-center">
            {alphabet.map((letter) => {
              ...
              return hasTerms ? (
                <a key={letter} href={`#letter-${letter}`} className="w-8 h-8 flex items-center justify-center text-sm font-medium rounded hover:bg-primary-600 hover:text-white transition-colors text-gray-700">{letter}</a>
              ) : (
                <span key={letter} className="w-8 h-8 flex items-center justify-center text-sm text-gray-300">{letter}</span>
              );
            })}
            {termsByLetter["#"] && (
              <a href="#letter-num" className="w-8 h-8 flex items-center justify-center text-sm font-medium rounded hover:bg-primary-600 hover:text-white transition-colors text-gray-700">#</a>
            )}
          </div>
        </section>
        ...
        <section className="max-w-5xl mx-auto px-4 pb-20">
          {termsByLetter["#"] && (
            <div id="letter-num" className="mb-10 scroll-mt-32">
              <h2 className="text-2xl font-bold text-gray-900 border-b-2 border-primary-600 pb-2 mb-6">#</h2>
          ...
          {alphabet.map((letter) =>
            termsByLetter[letter] && (
              <div key={letter} id={`letter-${letter}`} className="mb-10 scroll-mt-32">
                <h2 className="text-2xl font-bold text-gray-900 border-b-2 border-primary-600 pb-2 mb-6">{letter}</h2>
...
function TermCard({ term }: { term: (typeof GLOSSARY_TERMS)[number] }) {
  return (
    <Link href={`/glossary/${term.slug}`} className="block p-5 rounded-lg border border-gray-200 hover:border-primary-500 hover:shadow-md transition-all group">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="font-semibold text-gray-900 group-hover:text-primary-600 transition-colors">{term.term}</h3>
